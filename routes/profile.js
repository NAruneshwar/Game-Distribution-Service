...
router.get('/:user_id', async (req, res) => {
    ...
    try{
        ...
        res.status(401).render("posts/profile",{title: "Your profile", message: e})
    }
});

router.delete('/delete/:user_id',async(req,res)=>{ //for admin only... sessions required
    //Delete a player's profile.
    try {
        let user_id = req.params.user_id
        const deletedProfile = await booksData.deleteProfileById(user_id)
        if (deletedProfile == 1) {
            res.render("posts/admin-homepage", { title: "Admin Homepage", message: "Profile Deleted" });
        }
    } catch (e) {
        console.log(e)
